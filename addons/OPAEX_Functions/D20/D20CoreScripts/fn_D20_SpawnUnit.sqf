...
/*
This function spawns a unit, acting as a wrapper for createUnit
...
//Validate we're not spawning vehicle that aren't of the CAManBase type.
//Note, we changed to using Man as the class to check so that animals work.
if (not (_unitClassName isKindOf "Man")) exitWith
{
	[_group, _unitClassName, _position] call OPAEX_fnc_D20_SpawnVehicle //we'll return this too.
};
...
//Create unit
_newUnit = objNull;
_newUnit = _group createUnit [_unitClassName, _position, [], 0, "NONE"];
