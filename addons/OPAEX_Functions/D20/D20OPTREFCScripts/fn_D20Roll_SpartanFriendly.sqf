...
/*
D20 grenade roll: Spawns a friendly spartan.
...
_Side = independent; //We default to independent side as it is possible that _Side never gets set when _throwerSide returns a nil.
switch (_throwerSide) do {
    case west: {_Side = west};
    case east: {_Side = east};
    default {_Side = indFor}; //Default to independent
};
