//Jack Moss, aka Spartanjackwar.
/*
Function to generate a set of terrain points in a half-circle.  Useful for terrain manipulating.
A circle is defined as (x - h)^2 + (y - k)^2 = r^2, where r is radius and (h,k) is the origin.
Therefore, y = sqrt(r^2 - (x - h)^2) + k is a half-circle since +-(y - k)^2 applies to get full circles.

Parameter 0: position: position to center on.  This is our perceived [0, 0] on a coordinate grid.
Parameter 1: float: Height change relative to the existing terrain at each point.
Parameter 2: float: circular diameter.  Negatives are converted to absolute values.
Parameter 3: bool: Should we invert the semicircle?
Parameter 4: float: Length of the loop step.  Leave as 1 unless you have a need for sub-meter diameters.
Return: array of 3D positons.

Example:
	private _pos = screenToWorld [0.5, 0.5];  
	private _points = [_pos, -3, 5, false] call OPAEX_fnc_Terrain_HalfCircle; 
	setTerrainHeight [_points];

Can be executed in a non-scheduled environment.
Can be executed on any machine.
*/
params [
	[
		"_origin",
		[0,0,0],
		[[0,0,0]]
	],
	[
		"_heightChange",
		-3,
		[1.0]
	],
	[
		"_diameter",
		3,
		[1.0]
	],
	[
		"_invert",
		false,
		[true]
	],
	[
		"_stepLength",
		1,
		[1.0]
	]
];

private _newPositions = [];

//Check the bounds diameter for negatives
if (_boundsDiameter < 0) then {
	_boundsDiameter = abs(_boundsDiameter);
};

//Split the bounds diameter for the radii.
private _radius = _diameter / 2;
private _negRadius = -_radius;

//Generate the points.
for "_xStep" from _negRadius to _radius step _stepLength do {
	private ["_newHeight", "_nextPoint", "_yStep"];
	
	//Find Y.  We're using y = sqrt(r^2 - (x - h^2)) + k. for this function.
	if (!_invert) then {
		_yStep = sqrt(abs(_radius^2 - (_xStep)^2));
	} else {//Note that inverting the half-circle means -1 * (y - k) = -1 * sqrt(r^2 - (x - h)^2)
		_yStep = -sqrt(abs(_radius^2 - (_xStep)^2));
	}; //The absolute value of the sqrt internals is needed since sqrt has domain (0-inf)
	
	//Offset from the origin the X and Y steps.
	_nextPoint = _origin vectorAdd [_xStep, _yStep, 0];
	
	//Determine Z, which is the terrain height.
	_newHeight = (getTerrainHeight _nextPoint) + _heightChange;
	_nextPoint set [2, _newHeight];
	
	//Append the positions array with the [x, y, z] info.
	_newPositions pushBack _nextPoint;
};

_newPositions //return statement.
