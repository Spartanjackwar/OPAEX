...
/*
This function calculates the center of the top face of a box formed from a line.
While intended for bounding boxes, we can actually find the maximum height midpoint between any two points.
...
private _width  = (_boxMid select 0) / 2;
private _length = (_boxMid select 1) / 2;
private _height = (_boxMid select 2); //Note, that we don't need to half since we just want the top face.
